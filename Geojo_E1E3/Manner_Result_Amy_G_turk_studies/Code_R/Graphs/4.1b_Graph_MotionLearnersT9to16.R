...
# ggplot(data = df.means, aes(x=condition, y= lexBias*100, fill= condition,)) + geom_bar(aes(fill= condition), width = .7, stat = "identity") + ylim(0, 100) + guides(fill = FALSE) + xlab("Verb learning condition") + ylab("Percent of Means conjectures") + ggtitle("Lexicalization Bias for CoS Events (Trials 9-16)") + theme_bw()


# ggplot(data = df.means, aes(x=condition, y= lexBias*100, fill= condition,)) + geom_bar(aes(fill= condition), width = .7, stat = "identity") + ylim(0, 100) + guides(fill = FALSE) + xlab("Verb learning condition") + ylab("Percent of Means conjectures") + ggtitle("MM vs Path learners: Bias for CoS Verbs (Trials 9-16)") + theme_bw()
